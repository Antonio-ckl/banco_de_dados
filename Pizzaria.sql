...
* conhecidas como funções estaticas, as funções de agregação obtem 
* informação sobre conjuntos de linha especificas
...
-- qual é o valor da pizza mais barata do cardapio?
select min(valor) from pizzas;
-- qual é o valor do pedido numero 7?
...
select pedido_id as pedido, sum(valor) as 'valor pedido', sum(quantidade) as 'quantidade', avg(valor) as media, sum(valor) / sum(quantidade) as 'valor medio'from itens_pedido group by pedido_id;
...
select nome, count(*) from clientes inner join pedidos on clientes.id = pedidos.cliente_id group by nome;
select * from pedidos;
...
-- 2 quantos pedidos foram feitos pelo cliente "Alexandre Santos"

	select nome, count(clientes.id) from clientes inner join pedidos on clientes.id where nome= "Alexandre Santos";

-- 3 qual o valor de todos os pedidos feitos ate agora
